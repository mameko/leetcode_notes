...
原题是简单的hash，但扔盒子那帮人会出这么一堆follow up。T：O(n × len) S: O(n \* len)
...
public List<List<String>> findDuplicate(String[] paths) {
    ...
    for (String path : paths) {
        ...
        for (int i = 1; i < parts.length; i++) {
            String[] nameAndContent = parts[i].split("\\(");
            String name = nameAndContent[0]; // 1.txt
            String content = nameAndContent[1].replace(")", ""); // abcd
